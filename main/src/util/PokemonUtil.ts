import { NatureCorrection, ThreeStats } from "./Types";
...
const calcHp = (baseStats: number, level: number, threeStats: ThreeStats): number => {
  ...
  return Math.floor((baseStats * 2 + threeStats.individualValue + effortTemp) * level / 100) + level + 10;
};
...
const calcStatus = (baseStats: number, level: number, threeStats: ThreeStats): number => {
  ...
  const effortTemp = Math.floor(threeStats.effortValue / 4);
  const base = Math.floor((baseStats * 2 + threeStats.individualValue + effortTemp) * level / 100) + 5;
...
const getNatureCorrectionTitle = (goodStatusName: string, badStatusName: string): string | undefined =>
  natureCorrectionNameMap.get(goodStatusName)?.get(badStatusName);

export {
  getPokemonId, getPokemonUrl, getPokemonImgUrl,
  getTypeImage, calcHp, calcStatus,
  MAX_INDIVIDUAL_VALUE, MAX_EFFORT_VALUE,
  getIndividualValueLabel, getEffortValueLabel, getNatureCorrectionLabel,
